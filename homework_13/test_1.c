...
void updateDistances(int Prices[], link *cities[], int location,int before[])
{
    link *tmp = cities[location];
    while (tmp != NULL)
    {
        Prices[tmp->location] = Min(Prices[tmp->location], tmp->price + Prices[location]);
        if(Prices[tmp->location]==(tmp->price + Prices[location]))
        {
...
int computeMindistance(link *cities[], int n,int before[])
{
    int Prices[n + 1];
    Prices[1] = 0;
    int i = 0;
    ...
    newroad->next=NULL;
    newroad->price=0;
    updateDistances(Prices, cities, 1,before);
    while (1)
    {
        ...
        if (newInput == 0)
            break;
        flag[newInput] = 0;
        updateDistances(Prices, cities, newInput,before);
    }
    if (Prices[n] == 100000)
        return -1;
    else
...
int main()
{
    ...
    cityInput(cities,m);
    printf("%d",computeMindistance(cities,n,before));
    // i=n;
    // while(1)
    // {
    //     if(i==1) 
