...
double GetMinDistanceSum(double DistanceMatrix[][size],FILE*pf_w,city*cities[])
{
    ...
    for(i=1;i<size;i++)
    {
        Distances[i]=Min(Distances[i],DistanceMatrix[10][i]);
    }
    Sum+=Distances[29];
    Distances[29]=0;
    for(i=1;i<size;i++)
    {
        Distances[i]=Min(Distances[i],DistanceMatrix[29][i]);
    }
    fprintf(pf_w,"西宁 -- 郑州 : %fkm\n",DistanceMatrix[29][10]);
    for(i=3;i<size;i++)
    {
        int newInput=GetMinlocation(Distances,DistanceMatrix,pf_w,cities);
        if((newInput==16&&Distances[18]!=0)||(newInput==18&&Distances[16]))
        {
            Sum+=DistanceMatrix[16][18];
            Sum+=Distances[newInput];
            Distances[16]=0;
            Distances[18]=0;
            for(int j=0;j<size;j++)
            {
                Distances[j]=Min(Distances[j],DistanceMatrix[18][j]);
            }
            for(int j=0;j<size;j++)
            {
                Distances[j]=Min(Distances[j],DistanceMatrix[16][j]);
            }
            fprintf(pf_w,"%s -- %s : %fkm\n",cities[newInput]->name,cities[34-newInput]->name,DistanceMatrix[16][18]);            
        }
        else{
        Sum+=Distances[newInput];
